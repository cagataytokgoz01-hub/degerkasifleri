...
import { MAP_INFO } from "../data/program.js";
...
function renderMap(program, state) {
  ...
  const allDone = completedWeeks >= program.length;

  const mapIndex = getMapIndex(program, state);

  const info = allDone
    ? {
        short: "<strong>Tüm yolculuğu tamamladın.</strong>",
        long: "Çocuğunuz 13 haftalık değer yolculuğunu tamamladı. Bu süreçte farklı değerleri küçük hikâyeler ve uygulamalarla deneyimleme fırsatı buldu."
      }
    : (MAP_INFO[mapIndex] || MAP_INFO[1]);

  qs("mapScreen").innerHTML = `
    <img src="assets/images/m${mapIndex}.png" class="mapBg">

    <button id="mapInfoCard" class="mapInfoCard" type="button" aria-expanded="false">
      <div class="mapInfoShort">${info.short}</div>
      <div class="mapInfoLong">${info.long}</div>
    </button>
  `;

  const card = qs("mapInfoCard");
  if (!card) return;

  card.onclick = () => {
    const isOpen = card.classList.toggle("isOpen");
...
function renderAbout() {
    ...
    <div class="aboutWrap">
      ...
      <div class="aboutCard">
        ...
        <ul>
          ...
          <li>Ardından hikayeye eşlik eden uygulama yapılır.</li>
          <li>Günde 1 etkinlik yapılması önerilir.</li>
      ...
      <div class="aboutCard">
        ...
        <ul>
          ...
          <li>Her haftada 7 günlük içerik bulunur.</li>
          <li>Hikaye ve uygulamalar birlikte ilerler.</li>
  ...
  <h3>Amaç</h3>
  <div class="aboutDesc">
    Çocukların değerleri hikayeler ve uygulamalar aracılığıyla tanımalarını, hissetmelerini ve günlük yaşamda deneyimlemelerini desteklemek.
  </div>
</div>
...
if (actionBtn) {
  actionBtn.onclick = () => {
    ...
    else if (phase === "dayLocked") dispatch({ type: "SET_VIEW", view: "map" });
...
<button class="primaryBtn" id="actionBtn">Uygulamaya Geç</button>    `;
  ...
  `;
    ...
    `;
  } else if (phase === "dayLocked") {
    taskScreen.innerHTML = `
      <div class="card">
        <div class="bubbleText">
          <strong>Bugünü tamamladın! 🌟</strong>
        </div>
        <div class="noteText" style="text-align:center; margin: 12px 0 20px;">
          Yarın yeni bir adım seni bekliyor.<br>
          Her gün bir adım, yolculuk böyle ilerliyor.
        </div>
        <button class="primaryBtn" id="actionBtn">Tamam</button>
      </div>
    `;
