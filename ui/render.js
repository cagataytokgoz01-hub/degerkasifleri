...
function renderBadges(state) {
  ...
  let html = `
    ...
    <div style="position:absolute; inset:0; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:24px; box-sizing:border-box;">
      <div style="display:flex; justify-content:center; margin-bottom:40px;">
        <img src="assets/images/${finalBadge}" style="width:120px;">
      </div>
  `;
  ...
  rows.forEach(count => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      html += `<img src="assets/images/${img}" style="width:60px;height:60px;">`;
      week++;
    }
...
const backBtn = qs("backBtn");
  ...
  const lockedReplayBtn = qs("lockedReplayBtn");
  ...
  `;
    ...
    taskScreen.innerHTML = `
      <div class="card">
        ...
        <button class="primaryBtn" id="nextBtn">Devam</button>
      </div>
    `;
  } else if (phase === "lockedInfo") {
    taskScreen.innerHTML = `
      <div class="card">
        <div class="bubbleText">
          Bugünün etkinliğini tamamladın.<br>
          Yarın yeni bir etkinlik seni bekliyor.
        </div>
        <button class="primaryBtn" id="lockedReplayBtn">Bugünün Etkinliğini Tekrar Gör</button>
      </div>
    `;
...
export function renderApp(program, state, dispatch) {
  ...
  if (state.ui.view === "task") {
    ...
    if (state.ui.view === "badges") renderBadges(state);
